...
#' 
#' 
#' data %>% edar::gge_fit(., model, 'y', 'x1')
#' data %>% edar::gge_fit(., model, 'y', 'x1', facet1="cat.var2")
#' data %>% edar::gge_fit(., model, 'y', 'x1', cat.var='cat.var', cat.vale='d')
#' 
#' data %>% edar::gge_fit(., model, 'y', 'x2')
#' data %>% edar::gge_fit(., model, 'y', 'x2', col.group='cat.var2')
#' data %>% edar::gge_fit(., model, 'y', 'x2', facet1="cat.var2")
#' 
...
#'
#' This function uses \link{broom::tidy} to create a data.frame with model results from \code{lm} or \code{glm} functions
#'
#'
...
#'
#' @inheritParams tidye
#' @param type a string with the method to compute the standard errors (see \link{car::hccm}) 
#'
#' @export
## }}}
...
plot_residualsDiagnostic <- function(model)
{
    ...
    plot(model, col="#00000044")
}
plot_partialRegression <- function(model)
{
    par.default <- par(no.readonly = TRUE)
    on.exit(par(par.default))
    par(las=1,cex.axis=.7,bty='l',  cex.main=.9, mar=c(4,4,1,1), mgp = c(2,.6,0))
...
plot_partialResidual <- function(model)
{
    ...
    k = length(terms(model))
    m = .edar_get_layout(k)
    par(mfrow=c(m))

    coef <- names(model$coeff)
    par(las=1,cex.axis=.7,bty='l', pch=20, cex.main=.9, mar=c(4,4,3,1), mgp = c(2,.6,0), pch=19, col="#00000044")
