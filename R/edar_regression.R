...
#'
#'
#' @param data data frame containing the data used to fit the model.
#' @param smry a data frame with the summary of the model containing at least two columns named \code{term} and \code{estimate}. The column \code{estimate} must contains the point estimate. The columns \code{term} must match the name of the covariate associated the estimates as produced by \code{attr(terms(formula), 'term.labels')}, and \code{formula} must also be provided as a parameter.
#' @param formula an object of the class \code{formula} as used in the \code{lm} function. It must be the formula used to fit the model.
#' @param newdata either \code{NULL} (default) or a data frame with values to be used to produce the fitted values. If \code{NULL}, the new data is generated by the function \code{edar_get_new_data}
#' @inheritParams edar_get_new_data
...
edar_get_fitted <- function(data, smry, formula, newdata=NULL, x=NULL, n=NULL, cat.values)
{
    ...
    if (any(cat.vars %in% all.vars(formula))) {
        ...
        Xp = newdata %>% dplyr::bind_rows(., factors) %>%
